					       ...
					       const struct nf_hook_state *state)
{
	...
	rekernel_report(NETWORK, ip_hdr(socket_buffer)->version,
			data_len, NULL, uid, NULL, true, NULL);
	return NF_ACCEPT;
}
...
static int start_rekernel(void)
{
	...
	for (netlink_unit = NETLINK_REKERNEL_MIN;
		...
		netlink_socket = netlink_kernel_create(&init_net,
						       netlink_unit, &cfg);
		if (netlink_socket)
			break;
	}
		     ...
		     struct binder_transaction_data *tr)
{
	...
	if (reporttype == NETWORK) {
		char net_kmsg[PACKET_SIZE];

		snprintf(net_kmsg, sizeof(net_kmsg),
			 "type=Network,target=%d,proto=ipv%d,data_len=%d;",
			 dst_pid, type, src_pid);
		sendMessage(net_kmsg, strlen(net_kmsg));
		return;
	}
#endif
	...
	if (src && task_uid(src).val == task_uid(dst).val)
		return;

	switch (reporttype) {
	case BINDER:
		if (oneway && type == TRANSACTION) {
			if (!tr || tr->code < 29 || tr->code > 32)
				return;
			...
			snprintf(binder_kmsg, sizeof(binder_kmsg),
				 ...
				 binder_type[type], oneway, src_pid,
				 src ? task_uid(src).val : 0, dst_pid,
				 task_uid(dst).val, buf, tr->code);
		} else {
			snprintf(binder_kmsg, sizeof(binder_kmsg),
				 "type=Binder,bindertype=%s,oneway=%d,from_pid=%d,from=%d,target_pid=%d,target=%d,rpc_name=%s,code=-1;",
				 binder_type[type], oneway, src_pid,
				 src ? task_uid(src).val : 0, dst_pid,
				 task_uid(dst).val, rpc_type[type]);
		}
		break;
	case SIGNAL:
		snprintf(binder_kmsg, sizeof(binder_kmsg),
			 "type=Signal,signal=%d,killer_pid=%d,killer=%d,dst_pid=%d,dst=%d;",
			 type, src_pid, src ? task_uid(src).val : 0,
...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("Sakion Team & Timeline");
